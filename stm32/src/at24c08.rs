...
pub trait FromRawBytes<const LEN: usize> {
    fn from_raw_bytes(bytes : [u8; LEN]) -> Self;
}
...
pub struct Addresses<const LEN: usize, T: FromRawBytes<LEN>>{
    page: u8,
    offset: u8,
    phantom: PhantomData<T>,
}
impl<const LEN: usize, T: FromRawBytes<LEN>> Addresses<LEN, T> {
...
impl AT24C08 {
    ...
    fn get_address<const LEN: usize, T: FromRawBytes<LEN>>(
        &self,
    ...
    pub async fn read<const LEN: usize, T: FromRawBytes<LEN>>(
        &mut self,
        address: Addresses<LEN, T>,
    ...
    pub async fn write<const LEN: usize, T: FromRawBytes<LEN>>(
        &mut self,
        address: Addresses<LEN, T>,
        value: &[u8; LEN],
        ...
        let r = I2C
            ...
            .transaction(
                address[0],
                &mut [Operation::Write(&[address[1]]), Operation::Write(value)],
            )
            .await;
