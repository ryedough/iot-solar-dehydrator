...
use crate::{channeled_signal::SignalReceiver, esp12f::*, sht31::SHT31Reading};
...
impl ESP12FConsumer {
    ...
    pub async fn send_climate(&self, climate : SHT31Reading) -> SendClimateReturn {
        self.send.send(ESP12Command::SendClimate(climate)).await;
        let recv = self.recv.receive().await;
        if let ESP12FResult::SendClimate(res) = recv {
            res
        } else {
            panic!("this shouldnt possible unless you messed up esp12 task");
        }
...
pub struct ESP12FExecuteCommandTaskMaker {
...
impl ESP12FExecuteCommandTaskMaker {
    pub const fn new(
        result_ch: [Channel<ThreadModeRawMutex, ESP12FResult, 1>; ESP12F_CONSUMER_LEN],
        command_ch: [Channel<ThreadModeRawMutex, ESP12Command, 1>; ESP12F_CONSUMER_LEN],
    ) -> Self {
    ...
    pub fn create_esp12f_execute_command_task(
        &'static self,
        spawner: &Spawner,
        esp12f: ESP12F,
    ) -> [ESP12FConsumer; ESP12F_CONSUMER_LEN] {
        let consoomer: [ESP12FConsumer; ESP12F_CONSUMER_LEN] =
            core::array::from_fn(|i| ESP12FConsumer {
                recv: self.result_ch[i].receiver(),
                send: self.command_ch[i].sender(),
            });
        let producers: [ESP12FProducer; ESP12F_CONSUMER_LEN] =
            core::array::from_fn(|i| ESP12FProducer {
...
#[task]
async fn esp12f_execute_command_task(
    mut esp12f: ESP12F,
    sender: [ESP12FProducer; ESP12F_CONSUMER_LEN],
) -> ! {
            ...
            match s.recv.try_receive() {
                Ok(command) => {
                    ...
                    let result = match command {
                        ESP12Command::Scan => ESP12FResult::Scan(esp12f.scan().await),
                        ESP12Command::Connect(wifi) => {
                            ESP12FResult::Connect(esp12f.connect(wifi).await)
                        }
                        ESP12Command::SendClimate(reading) => ESP12FResult::SendClimate(Ok(())),
                    };
...
#[task]
pub async fn send_climate_periodically(
    mut interval: Duration,
    mut climate_wr : watch::Receiver<'static, ThreadModeRawMutex, SHT31Reading, 2>,
    send_climate_interval_sr: SignalReceiver<Duration>,
    esp12f_consumer : ESP12FConsumer,
) {
    loop {
        Timer::after(interval).await;
        let climate = climate_wr.get().await;
        if let Err(e) = esp12f_consumer.send_climate(climate).await {
            defmt::error!("Cant send climate, reason :{}", e);
        };
    };
}
